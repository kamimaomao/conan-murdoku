...
try {
  ...
  await page.getByRole('heading', { name: '阿笠博士家的可乐失踪' }).waitFor();

  await page.getByRole('button', { name: '第 1 行第 2 列' }).click();
  await page.locator('button[aria-label="第 1 行第 2 列"] .cell-object', { hasText: '冰箱' }).waitFor();

  await page.getByRole('button', { name: '江户川柯南' }).click();
  await page.getByText('柯南最后停在第 4 行第 4 列的足球旁。').waitFor();
  ...
  await page.getByRole('button', { name: '第 4 行第 4 列，江户川柯南' }).waitFor();
  await page.getByRole('button', { name: '第 2 行第 5 列，毛利兰' }).waitFor();
  await page.getByRole('button', { name: '第 3 行第 1 列，吉田步美' }).waitFor();
  await page.getByRole('button', { name: '第 5 行第 3 列，圆谷光彦' }).waitFor();
  await page.getByRole('button', { name: '第 1 行第 2 列，小岛元太' }).waitFor();
